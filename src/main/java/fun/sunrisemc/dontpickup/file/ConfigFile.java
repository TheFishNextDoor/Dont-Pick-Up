...
public class ConfigFile {
    ...
    public static boolean create(@NonNull String filename) {
        try {
            DontPickUpPlugin.getInstance().saveResource(filename + ".yml", false);
        } catch (Exception e) {
            DontPickUpPlugin.logWarning("Failed to save default configuration for " + filename + ".yml.");
            e.printStackTrace();
            return false;
        }
        return true;
    ...
    private static YamlConfiguration getDefault(@NonNull String name) {
        ...
        try {
            ...
            DontPickUpPlugin.logWarning("Failed to load default configuration for " + name + ".yml.");
            e.printStackTrace();
        }
